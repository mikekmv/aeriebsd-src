...
void
a2lprintf(const char *fname, const char *funame, int ln, int flags)
{
	char *p;
    ...
    int *pln)
{
	...
	if (IS_ELF(head.elf32) &&
		...
		if (!(shdr = elf32_load_shdrs(name, fp, 0, &head.elf32)))
			return 1;

		elf32_fix_shdrs(&head.elf32, shdr);
		if (!(shstr = elf32_shstrload(name, fp, 0, &head.elf32, shdr)))
			return 1;
		...
		if (i < head.elf32.e_shnum) {
			if (fseeko(fp, shdr[i].sh_offset, SEEK_SET)) {
				warn("%s: fseeko", name);
				free(shstr);
				free(shdr);
				return 1;
		...
		if (!(shdr = elf64_load_shdrs(name, fp, 0, &head.elf64)))
			return 1;

		elf64_fix_shdrs(&head.elf64, shdr);
		if (!(shstr = elf64_shstrload(name, fp, 0, &head.elf64, shdr)))
			return 1;
